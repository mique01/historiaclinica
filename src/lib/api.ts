...
export const api = {
  async getPatient(patientId: string) {
    return (await createClient().from('patients').select('*').eq('id', patientId).maybeSingle()).data;
  },
  async getDocuments(patientId: string) {
    return (
      (await createClient().from('documents').select('*').eq('patient_id', patientId).order('document_date', { ascending: false })).data ?? []
    );
  },
