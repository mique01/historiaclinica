...
export async function POST(_: Request, { params }: { params: Promise<{ attachmentId: string }> }) {
  ...
  const { data: att } = await supabase
    ...
    .select('*, inbox_messages(patient_id,sender_email,received_at)')
    .eq('id', attachmentId)
    .maybeSingle();

  if (!att) return NextResponse.json({ error: 'attachment_not_found' }, { status: 404 });
  if (att.inbox_messages.patient_id !== patientId) return NextResponse.json({ error: 'forbidden' }, { status: 403 });
  ...
  await supabase.from('documents').insert({
    ...
    uploaded_by: user.id,
    source: 'INBOX',
    doc_type: att.suggested_type ?? 'informe',
    title: att.filename,
    document_date: att.inbox_messages.received_at,
